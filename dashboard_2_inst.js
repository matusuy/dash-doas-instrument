...
// Developed by Matías Osorio (matusuy): mosorio@fing.edu.uy
...
function pintarDiv(id, alerta){
  ...
  // alerta 0: Todo OK
  // alerta 2: para discriminar entre medidas normales y de calibracion
  // alerta -1: No hay dato nuevo, alerta!
  
  //console.log(`pintarDiv called with id: ${id}, alerta: ${alerta}`);
  ...
  }else if (alerta==2){ // CALIBRATION ALERT --> AZUL
	document.getElementById(id).style.borderColor = "#3976ad";
    document.getElementById(id).style.backgroundColor = "#3976ad";
    document.getElementById(id).style.color = "white";	
...
function csvToArray(str, delimiter = " ") {
  ...
  var TempOutdoor           = parseFloat(ultima_fila["tele_temp_outside"]);           // OK -> deberia ser temperatura exterior
  var TempSpectrometer      = parseFloat(ultima_fila["sbox_temp_spectrometer"]);      // OK
  var HumidityOnElectronics = parseFloat(ultima_fila["sbox_humidity"]);               // OK 
  var TempElectronics       = parseFloat(ultima_fila["sbox_temp"]);                   // OK -> electronica en gral  
//  var TempSpectrometerNoise = parseFloat(ultima_fila["TempSpectrometerNoise"]);
  var DewPointSpectrometer  = parseFloat(ultima_fila["sbox_dewpoint"]);               // OK
  var SupplyVoltage         = parseFloat(ultima_fila["sbox_supply_voltage"]);         // OK -> solo caja de espectrometro
  var CurrentTotal          = parseFloat(ultima_fila["instrument_supply_current"]);   // OK
  var PeltierPower          = parseFloat(ultima_fila["sbox_peltier_power"]);          // OK 
  var SupplyVoltageTele     = parseFloat(ultima_fila["tele_supply_voltage"]);         // OK -> supply telescopio
  var UvSpecMin             = parseFloat(ultima_fila["uv_spec_min"]);
  var UvSpecMax             = parseFloat(ultima_fila["uv_spec_max"]);
  var UvSpecFilename        = ultima_fila["uv_spec_filenumber"];
  var VisSpecMin            = parseFloat(ultima_fila["vis_spec_min"]);
  var VisSpecMax            = parseFloat(ultima_fila["vis_spec_max"]);
  var VisSpecFilename       = ultima_fila["vis_spec_filenumber"];
  var MeasType              = ultima_fila["measurement_type"];
  
  //console.log(ultima_fila["measurement_type"]);
  
  var ultima_fila_menos_1 = arr[arr.length-2];
  var ultima_fila_menos_2 = arr[arr.length-3];
  
  var ElevationAngle1 = parseFloat(ultima_fila_menos_2["elevmotor_pos"]);             // OK
  var ElevationAngle2 = parseFloat(ultima_fila_menos_1["elevmotor_pos"]);             // OK
  var ElevationAngle3 = parseFloat(ultima_fila["elevmotor_pos"]);                     // OK
  
  var ElevationAngle_tele   = parseFloat(ultima_fila["tele_elevsens_angle"]);         // OK -> sensor en espejo
  ...
  var [day, month, year] = ultima_fila["date"].split('.');                // OK
  var [hh, mm, ss] = ultima_fila["time"].split(':');                      // OK
  ...
  document.getElementById('DewPointSpectrometer').innerHTML  = DewPointSpectrometer.toString() + " ºC";
  ...
  document.getElementById('SupplyVoltageTele').innerHTML     = SupplyVoltageTele.toString() + " V";
  document.getElementById('UvSpecMin').innerHTML             = UvSpecMin.toString() + "";
  document.getElementById('UvSpecMax').innerHTML             = UvSpecMax.toString();
  document.getElementById('UvSpecFilename').innerHTML        = UvSpecFilename.toString();
  document.getElementById('VisSpecMin').innerHTML            = VisSpecMin.toString();
  document.getElementById('VisSpecMax').innerHTML            = VisSpecMax.toString();
  document.getElementById('VisSpecFilename').innerHTML       = VisSpecFilename.toString();
  document.getElementById('MeasType').innerHTML              = MeasType.toString();
  ...
  document.getElementById('SupplyTeleAlert').innerHTML = '';
  ...
  document.getElementById('CalibrationAlert').innerHTML    = '';
  ...
  if (MeasType === 'calibration') {CalibrationAlert_msg = CalibrationAlert_msg + "\nCalibration lamp";}
  
  if (CalibrationAlert_msg === ''){
    pintarDiv('Measurements', 0);
  }else{
    pintarDiv('Measurements', 2);
    document.getElementById('CalibrationAlert').innerHTML = CalibrationAlert_msg;
    console.log(CalibrationAlert_msg);	
  }
  ...
  if (SupplyVoltageTele > 12.5) {SupplyTeleAlert_msg = SupplyTeleAlert_msg + "\nPotential problem of tele voltage";}
  if (SupplyVoltageTele < 11.4) {SupplyTeleAlert_msg = SupplyTeleAlert_msg + "\nPotential problem of tele voltage";}
  ...
  if (SupplyTeleAlert_msg === ''){
    pintarDiv('Supply', 0);
  }else{
    pintarDiv('Supply', 1);
    document.getElementById('SupplyTeleAlert').innerHTML = SupplyTeleAlert_msg;
    console.log(SupplyTeleAlert_msg)
  }
